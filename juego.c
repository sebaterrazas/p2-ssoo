...
int revisar_tablero(int x_i, int y_i, int x_f, int y_f, int largo_barco)
{
    ...
    if (jugador[y_i][x_i] == 0)//Punto inicial disponible
    {
        ...
        if (x_i == x_f) //Revisamos columna
        {
            ...
            orientacion = 0;
            if (y_i > y_f) //Coordenadas se dan al reves
            {
                for (h = 0; h < largo_barco - 1; h++)
                {
            ...
            else //Coordenadas en sentido correcto
            {
        ...
        if (y_i == y_f) //Revisamos la fila
        {
            ...
            orientacion = 1;
            if (x_i > x_f) //Coordenadas se dan al reves
            {
                for (h = 0; h < largo_barco - 1; h++)
                {
    ...
    {
        if (orientacion == 0) //Columna
        {
            if (posicion_final_x1 == x_f && posicion_final_y1 == y_f) //Columna hacia abajo
            {
                poner_barco(x_i,y_i,largo_barco,orientacion);
                imprimir_tablero();
                return 0;
            }  
            if (posicion_final_x1 == x_f && posicion_final_y2 == y_f) //Columna hacia arriba
            {
                poner_barco(x_f,y_f,largo_barco,orientacion);
                imprimir_tablero();
                return 0;
            }
            else // Las coordenadas no son validas
            {
                printf("Recuerda que el largo de tu barco es %d \n", largo_barco);
                return 1;
            }
        }
        if (orientacion == 1) //Fila
        {
            if (posicion_final_x1 == x_f && posicion_final_y1 == y_f) //Izquierda a derecha
            {
                poner_barco(x_i,y_i,largo_barco,orientacion);
                imprimir_tablero();
                return 0;
            }
            if (posicion_final_x2 == x_f && posicion_final_y1 == y_f) //Derecha a izquierda
            {
                poner_barco(x_f,y_f,largo_barco,orientacion);
                imprimir_tablero();
                return 0;
...
int main ()
{
    ...
    while (estas_seguro != 1)
    {
        ...
        printf("\t¿Estas segur@ de la posición de tus barcos?\n");
        printf("\t 1: Si, estoy seguro   0: No, deseo cambiarlo\n");
        scanf("%i", &respuesta);
        if (respuesta == 1)
        {
            estas_seguro = 1;
